...
export default function AttendancePage() {
  ...
  const filteredStudents = students.filter((s: any) =>
    (s.userDetails?.name || s.userId?.name || "").toLowerCase().includes(searchTerm.toLowerCase())
  )
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-slate-500">Mark daily attendance for all students</p>
        </div>
        <button
          onClick={handleSave}
          disabled={submitting}
                ...
                <tr key={student._id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        <div className="font-semibold text-slate-900">{student.userDetails?.name || student.userId?.name}</div>
